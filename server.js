...
async function start() {
  ...
  server.on("error", error => {
    if (error.code === "EADDRINUSE") {
      console.error(`Port ${PORT} is already in use. FlowGuard may already be running at http://localhost:${PORT}`);
      console.error(`Stop the existing server or run with a different PORT, for example: PORT=5174 npm run dev`);
      process.exit(1);
    }

    console.error("FlowGuard server error:", error);
    process.exit(1);
  });
